...
ggplot(defClean, aes(x=Statistic))+geom_histogram(binwidth = 1)+facet_wrap(~Type) + ylab("Number of Players")


```
Conclusion: From the plot, it shows Turnover_FR and Turnover_INT skills are most rare.
...
ggplot(TwoState, aes(x=West, y=KAN))+geom_point()
...
```{r}
#Using join for two data frames?
```
